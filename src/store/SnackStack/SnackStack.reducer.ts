...
export const SnackStackSlice = createSlice({
  ...
  reducers: {
    ...
    removeSnackBar: (state, action: PayloadAction<string>) => {
      state.snackBarsStack = state.snackBarsStack.filter((snack) => snack.id !== action.payload);
    },
  },
});

export const { showSnackBar, closeSnackBar, removeSnackBar } = SnackStackSlice.actions;

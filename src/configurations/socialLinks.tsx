import FacebookIcon from '@mui/icons-material/Facebook';
import GitHubIcon from '@mui/icons-material/GitHub';
import RedditIcon from '@mui/icons-material/Reddit';
import TelegramIcon from '@mui/icons-material/Telegram';
import TwitterIcon from '@mui/icons-material/Twitter';
import YouTubeIcon from '@mui/icons-material/YouTube';

import BitcoinIcon from '~/ui-kit/icons/BitcoinIcon';
import DiscordIcon from '~/ui-kit/icons/DiscordIcon';
import MediumIcon from '~/ui-kit/icons/MediumIcon';
import WeixinIcon from '~/ui-kit/icons/WeixinIcon';

export const socialLinks = [
  {
    title: 'discord',
    Icon: DiscordIcon,
    link: 'https://discord.obyte.org/',
  },
  {
    title: 'telegram',
    Icon: TelegramIcon,
    link: 'https://t.me/obyteorg',
  },
  {
    title: 'weixin',
    Icon: WeixinIcon,
    link: 'https://mp.weixin.qq.com/s/JB0_MlK6w--D6pO5zPHAQQ',
  },
  {
    title: 'twitter',
    Icon: TwitterIcon,
    link: 'https://twitter.com/ObyteOrg',
  },
  {
    title: 'youtube',
    Icon: YouTubeIcon,
    link: 'https://www.youtube.com/channel/UC59w9bmROOeUFakVvhMepPQ/',
  },
  {
    title: 'medium',
    Icon: MediumIcon,
    link: 'https://blog.obyte.org',
  },
  {
    title: 'reddit',
    Icon: RedditIcon,
    link: 'https://www.reddit.com/r/obyte/',
  },
  {
    title: 'bitcoin',
    Icon: BitcoinIcon,
    link: 'https://bitcointalk.org/index.php?topic=1608859.0',
  },
  {
    title: 'facebook',
    Icon: FacebookIcon,
    link: 'https://www.facebook.com/obyte.org',
  },
  {
    title: 'github',
    Icon: GitHubIcon,
    link: 'https://github.com/byteball/aa_stats',
  },
];
